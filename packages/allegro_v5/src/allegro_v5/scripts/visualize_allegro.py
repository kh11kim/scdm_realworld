"""
Live Allegro Hand visualizer using viser URDF rendering.

Data source: allegro_v5 multiprocessing server.
Assets: copied to ./assets/allegro_hand_description/{urdf,meshes}

Usage:
  python -m allegro_v5.scripts.visualize_allegro --socket-path /tmp/allegro_v5.sock --urdf right_B
"""
...
def main():
    ...
    parser.add_argument("--urdf", type=str, default="right_B", help="URDF name (default: right_B; also left_B/left_A/right_A)")
    args = parser.parse_args()

    urdf_path = resolve_urdf(args.urdf)

    telem = ZmqTelemetryClient(
        TelemetryConfig(
    ...
    urdf_model = yourdfpy.URDF.load(urdf_path, filename_handler=filename_handler)
    ...
    info_text = server.gui.add_markdown(f"**URDF:** {urdf_path}\n\nWaiting for telemetry...")
    sliders = {}
    with server.gui.add_folder("Joint position control"):
        for jname in joint_names:
            joint = urdf_model.joint_map[jname]
    ...
    def send_joint_cmd(desired: np.ndarray) -> None:
        nonlocal last_sent
    ...
    try:
        while True:
            ...
            if pos.shape[0] != len(joint_names):
                info_text.content = f"Received invalid joint array size: {pos.shape}"
                continue
            ...
            desired = np.array([sliders[name].value for name in joint_names], dtype=float)
            if sliders_synced and (last_sent is None or not np.allclose(desired, last_sent)):
